...
//!
//! During Task 4 only the bar path is compiled; xy marks still flow through
//! the legacy `render_xy`. `preflight()` is the shared spec/data validation
//! both paths run before dispatch.
...
use crate::scene::{
    Bar, Chrome, LegendEntry, Placed, Rect, Scene, SceneMark, Size, Source, XAxis, YAxis, YTick,
};
...
/// Resolve a spec into a Scene. Task 4 handles bars; xy marks still compile
/// via the legacy path in `render`, so they are unimplemented here.
pub fn compile(spec: &Spec, opts: &CompileOptions) -> Result<Scene, Error> {
    ...
    match spec.mark {
        ...
        Mark::Line | Mark::Point | Mark::Area => {
            todo!("xy marks compile in Task 5; render() uses the legacy path for now")
        }
    }
}
