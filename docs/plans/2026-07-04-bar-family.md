...
**Step 2: Rasterizer.** `rasterize_bars` fills rects generically:

- **Dots:** pixel ranges become two-dimensional:
...
- **Blocks:** keep the vertical fill path (bottom-up eighths via `EIGHTHS`)
  for bars with `y0 + h == 1.0` (full-drop bars); add a horizontal fill
  path (left-anchored bars, `x0 == 0.0`) using a new `LEFT_EIGHTHS` table
  (`▏▎▍▌▋▊▉█`, U+258F..U+2589 + U+2588) for the partial END column. Match
  on the bar's anchor, not a flag — the Scene stays orientation-free.
...
and nothing else changes in the file (verify by script: the diff per bar
is exactly one added line). **Zero gallery diffs** — this is the referee
proving byte-identical vertical rendering. Zero meta changes.
...
`xOffset`) an `Option<Channel>` field `x_offset`. It exists ONLY to be
rejected helpfully in validate:
...
**Step 2: Validate widening.** Replace the bar color==x check
(`compile.rs:94-100`): a bar color field different from x is now the
grouped case, not an error. Keep the equal-field tint path unchanged.

**Step 3: compile_bar grouping.** When `color.field != x.field`:
...
- **Layout:** per category slot `step = plot_w / n_cats`; group width
  `(step * 0.7).floor().max(n_series)` columns, split into `n_series`
  equal bar widths (min 1 col); bars packed adjacent within the group,
  group centered in its slot. Colors `theme.series(si)`.
...
`grouped_agg_mean`, `err_xoffset` (exact message), `err_grouped_palette`
(9 series, the chrome-cycle message verbatim).
...
- Create: `tests/cases/ranking_horizontal.json`, `horizontal_declared.json`
  ...
  `err_both_categorical.json`, `err_height_ceiling.json`,
  `name_truncation.json`
...
| nominal/ordinal | nominal/ordinal | error (see below)                  |
...
(Two variants, `quantitative` / `categorical`.) NOTE: today's `compile_bar`
treats x as categorical regardless; the resolution must not change any
existing case's route — every existing bar corpus case has categorical x
and numeric y. Verify zero existing-corpus diffs.
...
- **Content-sized height:** `plot_h = n_bars + gaps` where plain bars get
  one row each and one blank row between categories
  (`n_cats * 2 - 1` plain). The height from `opts.height`/`spec.height` is
  a CEILING: `content > ceiling` → error
  `"{n} bars need height {h}; filter or aggregate, or raise --height"`
  (kind data, exit 3). No height given → safety cap 40 rows, same message
  shape. `plot_dims` width handling unchanged; its height result feeds the
  ceiling check only.
...
- **Bars:** `Bar { x0: 0.0, y0: row / rows, w: xscale.norm(v),
  h: 1.0 / rows, color }` — gradient by `xscale.norm(v)` (plain) or
  `theme.series(i)` (tint, color==y-field).
